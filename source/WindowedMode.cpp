		...
		void* newFunction, void** originalFunction)
	{
		...
		for (; imports->Name; ++imports)
		{
			...
			for (; firstThunk->u1.Function; ++firstThunk, origThunk = origThunk ? (origThunk + 1) : nullptr)
			{
				...
				DWORD oldProtect;
				if (!VirtualProtect(&firstThunk->u1.Function, sizeof(uintptr_t), PAGE_EXECUTE_READWRITE, &oldProtect)) // prevent crashes when the module is protected against writing
					return false;
